...
class AverageGenerator:
    ...
    def list_generator(self):
        self.new_list = [random.randint(
            self.min_value, self.max_value) for i in range(self.list_size)]

    def find_average(self):
        self.average = self.new_list[0] if len(
            self.new_list) == 1 else sum(self.new_list) / len(self.new_list)
    ...
    def __str__(self):
        return f'Arithmetic mean of the list {self.new_list} \nequals {self.average}'
